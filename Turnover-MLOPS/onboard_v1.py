...
# O MLflow 3 imprime caracteres Unicode ao encerrar um run. No Windows, o
# terminal pode estar em cp1252 e falhar depois de o registro ja ter ocorrido.
...
def main():
    ...
    caminho_pkl = Path(args.pkl).expanduser()
    if not caminho_pkl.is_absolute():
        caminho_pkl = config.RAIZ / caminho_pkl
    caminho_pkl = caminho_pkl.resolve()
    if not caminho_pkl.is_file():
        raise SystemExit(f"Arquivo do modelo nao encontrado: {caminho_pkl}")

    with caminho_pkl.open("rb") as f:
    ...
    with mlflow.start_run(run_name="v1_onboarding_do_pkl") as run:
        ...
        mlflow.set_tags({
            ...
            "origem": "pre_mlflow",
            "lineage": "incompleto",
            "arquivo_origem": str(caminho_pkl),
        })
        ...
        destino_modelo = (
            {"name": "modelo"}
            if int(mlflow.__version__.split(".", 1)[0]) >= 3
            else {"artifact_path": "modelo"}
        )
